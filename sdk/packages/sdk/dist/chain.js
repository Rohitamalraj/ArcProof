 ...
 * config.
 */
import { createPublicClient, createWalletClient, http, defineChain, parseEther, formatEther, getAddress, } from "viem";
 ...
 */
export async function transfer(network, fromPrivateKey, toAddress, amountNative, memo = "") {
    ...
    console.log(`[chain] ${account.address.slice(0, 10)}.. -> ${toChecksum.slice(0, 10)}.. : ` +
 ...
 */
export async function verifyTransfer(network, txHash, expectedFrom, expectedTo, minAmountNative) {
    const client = createPublicClient({ chain: chainFor(network), transport: http(network.rpcUrl) });
    try {
