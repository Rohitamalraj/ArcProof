...
export interface ElizaModelRuntime {
  useModel: (
    modelType: string,
    params: { prompt: string; schema?: unknown; output?: "object" | "array" | "enum"; temperature?: number }
  ) => Promise<unknown>;
}
...
function runtimeFrom(context: Record<string, unknown>): ElizaModelRuntime {
  const rt = context[ELIZA_RUNTIME_CONTEXT_KEY] as ElizaModelRuntime | undefined;
  if (!rt || typeof rt.useModel !== "function") {
    throw new Error(
      "no ElizaOS runtime in context -- createEliza* gatherers must be run through createArcProofAction " +
 ...
 */
export function createElizaClaimGatherer(
  options: ElizaClaimGathererOptions
  ...
  return async function gatherClaims(context: Record<string, unknown>): Promise<Claim[]> {
    ...
    try {
      ...
      const prompt =
        `${options.systemPrompt}${claimTypeHint}\n\n` +
        `Request:\n${userMsg}\n\n` +
      ...
      const result = (await runtime.useModel(OBJECT_LARGE, { prompt, schema: CLAIMS_SCHEMA, output: "object", temperature: 0 })) as {
        claims?: ClaimDraft[];
      };
 ...
 */
export function createElizaOrchestrator(
  options: ElizaOrchestratorOptions
): (context: Record<string, unknown>) => Promise<Claim[]> {
  const specialistIds = options.specialists.map((s) => s.id);
  if (specialistIds.length === 0) throw new Error("createElizaOrchestrator: at least one specialist is required");
  ...
  return async function gatherClaims(context: Record<string, unknown>): Promise<Claim[]> {
    ...
    const plan = (await runtime.useModel(OBJECT_LARGE, {
      ...
      output: "object",
      temperature: 0,
    })) as { specialist_ids?: string[]; reasoning?: string };

    let selectedIds = (plan.specialist_ids ?? []).filter((id) => specialistIds.includes(id));
    if (!selectedIds.length) selectedIds = specialistIds;
